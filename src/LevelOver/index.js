...
import ElementSqare_Half_Size_webp from 'assets/images/ElementSqare.Half.Size.webp';
import phi15phi_svg from 'assets/images/phi15phi.svg';

import { gameLevels } from '../constants.js';

const songImgElem = document.createElement('img');
songImgElem.id = 'songImg';
songImgElem.class = 'songImg';
songImgElem.level = 'HD Lv.10';
songImgElem.src = ElementSqare_Half_Size_webp;
document.getElementsByClassName('leftPart')[0].appendChild(songImgElem);

const gradeImageElem = document.createElement('img');
gradeImageElem.id = 'gradeImage';
gradeImageElem.class = 'grade';
gradeImageElem.src = phi15phi_svg;
document.getElementById('scrollFrame').appendChild(gradeImageElem);
...
window.addEventListener('DOMContentLoaded', () => {
	...
	document.getElementById('backInResultBtn').addEventListener('click', () => {
		location.href = '../songSelect?c=' + parsedURLParams.get('c');
	});
	//	判断等级（范围来自萌娘百科）
	if (score == 0) {
	...
	songInfoXHR.addEventListener('load', () => {
		...
		xhr.open('GET', LevelOver_mp3, true);
		...
		document.querySelector('#score').innerText = score
			.toString()
			.padStart(7, '0');
		document.querySelector('#gradeImage').src = import(
