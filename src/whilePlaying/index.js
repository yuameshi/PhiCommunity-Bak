...
import { loadPhiCommunityResources } from './loadPhiCommunityResources';
...
window.onload = function () {
	if (window.ResourcesLoad != 100) {
		loadPhiCommunityResources(res, actx, message);
	}
	// //加载资源
	// (async function () {
	// 	const loadItems={
...
const loadFile = function (file) {
	...
	reader.onprogress = (progress) => {
		...
		message.sendMessage(
			`加载文件：${Math.floor((progress.loaded / size) * 100)}%`
		);
	};
	reader.onload = async function () {
		//加载zip//gildas-lormeau.github.io/zip.js)
		const reader = new zip.ZipReader(
			new zip.Uint8ArrayReader(new Uint8Array(this.result))
		);
		reader.getEntries().then(
			async (zipDataRaw) => {
				...
				console.log(zipData);
				let loadedNum = 0;
				const zipRaw = await Promise.all(
					zipData.map(
						(i) =>
							new Promise(async (resolve) => {
								if (i.filename == 'line.csv') {
									const data = await i.getData(
										new zip.TextWriter()
									);
									...
									i.getData(new zip.Uint8ArrayWriter())
										.then(async (data) => {
											const audioData =
												await actx.decodeAudioData(
													data.buffer
												);
											bgms[i.filename] = audioData;
											selectbgm.appendChild(
												createOption(
													i.filename,
													i.filename
												)
										...
										.catch(async () => {
											...
											const jsonData = await chart123(
												JSON.parse(data)
											);
											charts[i.filename] = jsonData;
											charts[i.filename]['md5'] =
												md5(data);
											selectchart.appendChild(
												createOption(
													i.filename,
													i.filename
												)
				...
				function loading(num) {
					message.sendMessage(
						`读取文件：${Math.floor((num / zipData.length) * 100)}%`
					);
...
function qwqdraw1(now) {
	...
	ctxos.setTransform(
		...
		lineScale *
			(qwqIn.second < 0.67
				? tween[2](qwqIn.second * 1.5) - 1
				: -tween[2](qwqOut.second * 1.5)) *
			1.75
	);
	...
	if (qwqIn.second < 3) {
		if (qwqIn.second < 0.67)
			ctxos.globalAlpha = tween[2](qwqIn.second * 1.5);
		else if (qwqIn.second >= 2.5)
			ctxos.globalAlpha = tween[2](6 - qwqIn.second * 2);
		ctxos.textAlign = 'center';
		//歌名
		...
		const imgW =
			lineScale *
			48 *
			(qwqIn.second < 0.67 ? tween[3](qwqIn.second * 1.5) : 1);
	...
	if (stat.combo > 2) {
		...
		ctxos.globalAlpha =
			qwqIn.second < 0.67
				? tween[2](qwqIn.second * 1.5)
				: 1 - tween[2](qwqOut.second * 1.5);
...
function qwqdraw3(statData) {
	...
	ctxos.fillText(
		inputName.value || inputName.placeholder,
		700 * tween[8](range(qwqEnd.second * 1.25)) - 320,
		145
	);
	ctxos.font = '30px Mina';
	ctxos.fillText(
		inputLevel.value || inputLevel.placeholder,
		700 * tween[8](range(qwqEnd.second * 1.25)) - 317,
		208
	);
	//Rank图标
...
function chartp23(pec, filename) {
	...
	class JudgeLine {
		...
		constructor(bpm) {
			this.bpm = bpm;
			'speedEvents,notesAbove,notesBelow,judgeLineDisappearEvents,judgeLineMoveEvents,judgeLineRotateEvents,judgeLineDisappearEventsPec,judgeLineMoveEventsPec,judgeLineRotateEventsPec'
				.split(',')
				.map((i) => (this[i] = []));
		}
...
const tween = [
	...
	(pos) =>
		pos < 0.5
			? 2 ** (20 * pos - 11) * Math.sin(((160 * pos + 1) * Math.PI) / 18)
			: 1 -
			  2 ** (9 - 20 * pos) * Math.sin(((160 * pos + 1) * Math.PI) / 18), //29
];
//导出json
function chartify(json) {
	...
	for (const i of json.judgeLineList) {
		...
		newLine.bpm = i.bpm;
		'speedEvents,notesAbove,notesBelow,judgeLineDisappearEvents,judgeLineMoveEvents,judgeLineRotateEvents'
			.split(',')
			.map((i) => (newLine[i] = []));
		for (const j of i.speedEvents) {
...
window.addEventListener('DOMContentLoaded', () => {
	...
	document
		...
		.addEventListener('click', () => {
			...
			playExitSoundXHR.open('GET', Exit_mp3, true);
...
function replay() {
	...
	try {
		...
		Renderer.chart = chart123(chartp23(window.chartString, undefined));
	}
	btnPlay.click();
}
document
